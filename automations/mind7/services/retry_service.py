...
from automations.mind7.constants.query import (CAPTCHA_RESULT, DOCUMENT_NOT_FOUND_VALUE, EMPTY_DOCUMENT_VALUE, MAX_RETRY_WAIT_SECONDS,RETRY_BATCH_SIZE,)
...
def _retry_round(page, worksheet, plate_column: int, document_column: int, pending_rows: list[int]) -> tuple[list[int], int]:
    ...
    for batch in _split_in_batches(pending_rows, RETRY_BATCH_SIZE):
        for row in batch:
            solved = _process_retry_row(page=page, worksheet=worksheet, row=row, plate_column=plate_column, document_column=document_column, captcha_rows=captcha_rows)
            if solved:
                solved_in_round += 1
            time.sleep(_random_short_wait())
...
def _process_retry_row(page, worksheet, row: int, plate_column: int, document_column: int, captcha_rows: list[int]) -> bool:
