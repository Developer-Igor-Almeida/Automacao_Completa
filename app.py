...
def monitorar_conexao():
    """Fecha processos filhos quando não houver conexão ativa no Streamlit.

    Útil para evitar que automações continuem abertas depois de fechar o navegador.
    """
    time.sleep(20)
...
def calcular_progresso(log_texto):
    ...
    if "Finalizado" in texto or "FINALIZADO" in texto or "finalizado" in texto.lower():
        return 100
...
if st.session_state.erro_pausado:
    ...
    if st.button("▶️ Continuar após correção", type="primary"):
        ...
        iniciar_processo(
            [sys.executable, "-u", "extrair_tracers.py"],
            TRACERS,
            LOG_TRACERS,
...
with col_dir:
    ...
    if not (MIND7 / "consultar_mind7.py").exists():
        ...
        if st.button(
            ...
            iniciar_processo(
                [sys.executable, "-u", "consultar_mind7.py"],
                MIND7,
                LOG_MIND7,
...
with st.expander("Status da execução", expanded=True):
    log_texto_corrigido = corrigir_acentos_log(ler_log(st.session_state.log_atual))
    mensagem = mensagem_amigavel_por_etapa(st.session_state.etapa, log_texto_corrigido)
